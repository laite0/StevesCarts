...
public class ArcadeMonopoly extends ArcadeGame {
	...
	public void drawBackground(final GuiMinecart gui, final int x, final int y) {
		...
		if (selectedPlace != -1) {
			drawPropertyOnBoardWithPositionRotationAndScale(gui, places[selectedPlace], selectedPlace, true, false, (int) ((590.6666666666666 - (
		...
		for (int i = 0; i < places.length; ++i) {
			if (!foundHover && getModule().inRect((int) smallgridX, (int) smallgridY, getSmallgridPlaceArea(i))) {
				if (selectedPlace == -1) {
					drawPropertyOnBoardWithPositionRotationAndScale(gui, places[i], i, true, false, (int) ((590.6666666666666 - ((getId(i) == 0) ? 122 : 76)) / 2.0), 51, 0, 0.75f);
				}
				foundHover = true;
		...
		for (int i = 0; i < pieces.size(); ++i) {
			...
			getModule().drawImage(gui, menu, 0, 122);
			for (int j = 0; j < 3; ++j) {
				int v = 0;
				switch (j) {
					case 0: {
				...
				getModule().drawImage(gui, menu[0] + 3, menu[1] + 3 + j * 9, j * 12, 152 + 6 * v, 12, 6);
			}
			final int[] player = piece.getPlayerMenuRect(i);
			getModule().drawImage(gui, player, 232, 24 * piece.getV());
		...
		for (final Button button : buttons) {
			if (button.isReallyVisible(this)) {
				...
				getModule().drawImage(gui, rect, 152, v * 18);
			}
		}
		if (getSelectedPlace() != null) {
			if (getSelectedPlace() instanceof Street) {
				final Street street = (Street) getSelectedPlace();
				getModule().drawImage(gui, 32, 185, 76, 22, 16, 16);
				if (street.getOwner() != null && !street.isMortgaged()) {
					if (street.getStructureCount() == 0) {
						getModule().drawImage(gui, 7, street.ownsAllInGroup(street.getOwner()) ? 241 : 226, 124, 22, 5, 10);
					} else {
						getModule().drawImage(gui, 323, 172 + (street.getStructureCount() - 1) * 17, 124, 22, 5, 10);
					}
				}
				for (int l = 1; l <= 5; ++l) {
					drawStreetRent(gui, street, l);
				}
				...
				if (station.getOwner() != null && !station.isMortgaged()) {
					getModule().drawImage(gui, 323, 184 + (station.getOwnedInGroup() - 1) * 17, 124, 22, 5, 10);
				}
				Note.drawValue(this, gui, 62, 170, 3, station.getMortgageValue());
				for (int l = 1; l <= 4; ++l) {
					drawStationRent(gui, station, l);
				}
				...
				if (utility.getOwner() != null && !utility.isMortgaged()) {
					getModule().drawImage(gui, 323, 184 + (utility.getOwnedInGroup() - 1) * 17, 124, 22, 5, 10);
				}
				Note.drawValue(this, gui, 62, 170, 3, utility.getMortgageValue());
				for (int l = 1; l <= 3; ++l) {
					drawUtilityRent(gui, utility, l);
				}
			}
		}
		if (currentCard != null) {
	...
	public void drawForeground(final GuiMinecart gui) {
		...
		for (final Button button : buttons) {
			if (button.isReallyVisible(this)) {
				getModule().drawString(gui, button.getName(), getButtonRect(id++), 4210752);
			}
		}
		if (getSelectedPlace() != null) {
			if (getSelectedPlace() instanceof Street) {
				getModule().drawString(gui, "Mortgage", 10, 175, 4210752);
				getModule().drawString(gui, "Buy", 10, 190, 4210752);
				getModule().drawString(gui, "Rents", 10, 215, 4210752);
				getModule().drawString(gui, "Normal", 14, 227, 4210752);
				getModule().drawString(gui, "Group", 14, 242, 4210752);
			} else if (getSelectedPlace() instanceof Station) {
				getModule().drawString(gui, "Mortgage", 10, 175, 4210752);
				getModule().drawString(gui, "Rents", 330, 170, 4210752);
			} else if (getSelectedPlace() instanceof Utility) {
				getModule().drawString(gui, "Mortgage", 10, 175, 4210752);
	...
	private void drawStreetRent(final GuiMinecart gui, final Street street, final int structures) {
		...
		for (int i = 0; i < graphicalStructures; ++i) {
			getModule().drawImage(gui, 330 + i * 6, yPos, 76 + u * 16, 22, 16, 16);
		}
	...
	private void drawStationRent(final GuiMinecart gui, final Station station, final int ownedStations) {
		...
		for (int i = 0; i < ownedStations; ++i) {
			getModule().drawImage(gui, 330 + i * 16, yPos, 76 + i * 16, 70, 16, 16);
		}
	...
	private void drawUtilityRent(final GuiMinecart gui, final Utility utility, final int utils) {
		...
		for (int i = 0; i < utils; ++i) {
			getModule().drawImage(gui, 330 + i * 16, yPos, 76 + i * 16, 86, 16, 16);
		}
	...
	private void drawPropertyOnBoardWithPositionRotationAndScale(final GuiMinecart gui, final Place place, final int id, final boolean zoom, final boolean hover, final int x, final int y, final int r, final float s) {
		...
		for (int j = 0; j < pieces.size(); ++j) {
			if (!pieces.get(j).isBankrupt() && pieces.get(j).getPosition() == id) {
				...
				place.drawPiece(gui, pieces.get(j), total[area], pos[area]++, area, states);
			}
		}
